...
$SaldoQuery = "select * from ent_documento where IdPais_Documento = '$paisEmpresa' AND IdEmpr_Documento = '$empresaID' AND IdTipoDoc_Documento = 1 AND Estatus_Documento='PP' AND IdTipoPago_Documento= 1 AND Codigo_Documento='$CodSocio' ";
...
elseif (isset($_POST['TablaPEPA'])) {
	$CodSocio = $_POST['Codigo'];
$SaldoQuery = "select * from ent_documento where IdPais_Documento = '$paisEmpresa' AND IdEmpr_Documento = '$empresaID' AND IdTipoDoc_Documento = 1  AND IdTipoPago_Documento= 1 AND Codigo_Documento='$CodSocio' ";
	   $SaldoQueryConsulta = $mysqli->query($SaldoQuery);
...
<td>
<input type="button" onclick="VerRecivos('.$RowSaldo['Id_Documento'].');" value="Ver Recivos"/></p>
</td>
</tr>';
}
$datos .= '
...
$MAXID=mysqli_query($mysqli,"select max(Id_Recibo)+1 as maximo from ent_recibo");
...
$Cerrado=0;
	...
	$Nuevoo=mysqli_query($mysqli,"INSERT INTO ent_recibo (`Id_Recibo`,Serie_Recibo, `NoDocto_Recibo`, `IdCliente_Recibo`, `Fecha_Recibo`, `Valor_Recibo`, `IdTipoPago_Recibo`, Cerrado_Recibo,`Usuario_Recibo`, `FechaCreacion_Recibo`, `MaqCreacion_Recibo`) VALUES ('$maxid','$Serie','$Idocumento','$Codigo','$fecha','$Cantidad','1','$Cerrado','$usuarioLog','$fecha', '$Maquina')");
