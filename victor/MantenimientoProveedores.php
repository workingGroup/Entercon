...
$querySeleccion="SELECT * FROM ent_maessocio where Id_MaesEmpr='$empresaID' && Id_MaesPais	 ='$paisEmpresa' && Tipo_MaesSocio= '2'  ORDER BY Id_Socio asc LIMIT 1 ";	
...
$query="SELECT * FROM ent_maessocio where Id_MaesEmpr='$empresaID' && Id_MaesPais	 ='$paisEmpresa' && Tipo_MaesSocio= '2' && Id_Socio='$idbuscar' or   Id_MaesEmpr='$empresaID' && Id_MaesPais	 ='$paisEmpresa' && Tipo_MaesSocio= '2'&& RazonSocial_MaesSocio ='$idbuscar'  ";	
...
///////////final
if ($accion=="Final"){
$FinalQuery = "SELECT MAX(Id_Socio)as idmax FROM ent_maessocio where Id_MaesPais='$paisEmpresa' && Id_MaesEmpr= '$empresaID' && Tipo_MaesSocio= '2'";
$respuestaFinal=$mysqli->query($FinalQuery);
$rowFinal=$respuestaFinal->fetch_assoc();
$maxID= $rowFinal['idmax'];

$query="SELECT * FROM ent_maessocio where Id_MaesEmpr='$empresaID' && Id_MaesPais ='$paisEmpresa' && Id_Socio = '$maxID' && Tipo_MaesSocio= '2'";	
$resultado=$mysqli->query($query);
...
///Siguiente	
 if ($accion=="Siguiente"){
			$valorIdSiguiente = $_POST["ID"];
			 $SiguienteQuery="SELECT * FROM ent_maessocio WHERE Id_Socio > '$valorIdSiguiente' && Id_MaesEmpr='$empresaID' && Tipo_MaesSocio= '2' && Id_MaesPais ='$paisEmpresa' ORDER BY Id_Socio asc LIMIT 1 " ;
...
if ($accion=="Previo"){
			$valorIdPrevio = $_POST["ID"];
			 $PrevioQuery="SELECT * FROM ent_maessocio WHERE Id_Socio < '$valorIdPrevio' && Id_MaesEmpr='$empresaID' && Tipo_MaesSocio= '2' && Id_MaesPais ='$paisEmpresa' ORDER BY Id_Socio desc LIMIT 1  ";
...
//inicio 
if ($accion =="inicio"){
	$inicioQuery ="SELECT Min(Id_Socio)as idmin FROM ent_maessocio where  Id_MaesPais='$paisEmpresa' && Tipo_MaesSocio= '2' && Id_MaesEmpr= '$empresaID'";
	$respuestaInicio = $mysqli->query($inicioQuery);
	$row=$respuestaInicio->fetch_assoc();
	$minID= $row['idmin'];
	
	
	$queryinicio="SELECT * FROM ent_maessocio where Id_MaesEmpr='$empresaID' && Id_MaesPais ='$paisEmpresa' && Id_Socio = '$minID' && Tipo_MaesSocio= '2' ";	
...
if($accion == "Modificar"){
	if($IDmodificar !=""){
		echo "<script> window.open('ModificarProveedor.php?id=$IDmodificar  &nit=$Nitmodificar &razon=$Razonmodificar &direccion=$Direccionmodificar & telefonos=$Telefonosmodificar & fax=$Faxmodificar & email=$Emailmodificar ','Modificar-Bodega','width=1100, height=500')</script> " ;}else {echo "No hay proveedor para Modificar";}
		}		
   
///agregar
if($accion=="Agregar"){
	echo "<script> window.open('NuevoProveedor.php', 'Nuevo-Cliente','width=1100, height=500')</script> ";
}
///Imprimir
if ($accion == "Imprimir"){
}
